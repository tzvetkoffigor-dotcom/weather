...
async def send_reminder(context):
    ...
    job = context.job
    await context.bot.send_message(job.chat_id, text="⏰ Время вышло!")

def parse_time(text: str, user_id: int):
    """
    Парсит сообщение и возвращает время в минутах
    """
    ...
    for unit_name, multiplier in all_units.items():
        fraction_pattern = rf'(\d+)/(\d+)\s*{re.escape(unit_name)}'
        ...
        if match:
            try:
                ...
                if denominator != 0:
                    ...
                    logger.info(f"Найдена дробь: {numerator}/{denominator} {unit_name} = {result} минут")
                    return result
    ...
    for unit_name, multiplier in all_units.items():
        number_pattern = rf'(\d+\.\d+|\d+)\s*{re.escape(unit_name)}'
        match = re.search(number_pattern, text_lower)
        if match:
            try:
                number = float(match.group(1))
                result = number * multiplier
                logger.info(f"Найдено число: {number} {unit_name} = {result} минут")
                return result
            except ValueError:
                continue
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Парсим время
    minutes = parse_time(text, user_id)
    ...
    logger.info(f"Распознано время: {minutes} минут")
    ...
    try:
        ...
        await update.message.reply_text(f"✅ Напоминание установлено на {time_display}")
